...
const PointMarker = ({ center, content, openPopup, L, origin, destination }) => {
  ...
  return origin || destination ? (
    <Marker ref={markerRef} position={center} icon={origin ? originnMarker : destinationMarker}>
      <Popup minWidth={300} position={position} keepInView={true} autoPan={true} className="map-popup-with-icon">
        <PopupMarkerContent content={content} />
      </Popup>
    </Marker>
  ) : (
    <Marker ref={markerRef} position={center}>
      <Popup minWidth={300} position={position} keepInView={true} autoPan={true} className="map-popup-inactive">
        <PopupMarkerContent content={content} />
      </Popup>
    </Marker>
  );
